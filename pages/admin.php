        ...
        <?php if ($page === 'admin'): ?>
            ...
            <?php } else { ?>
                ...
                if ($current_role === 'agency') {
                    ...
                    if (!in_array($tab, ['dashboard', 'users', 'activations', 'settings', 'contacts', 'payments', 'receipts', 'branding', 'articles', 'feedbacks']))
                        $tab = 'dashboard';
                ...
                <div class="admin-wrap">
                    <aside class="admin-sidebar">
                        ...
                        <?php if ($current_role === 'admin'): ?>
                            <a href="?tab=users" class="admin-menu-link <?= $tab == 'users' ? 'active' : '' ?>"><svg
                                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <a href="/trang-chu" class="admin-menu-link admin-sidebar-home"
                            style="margin-top:auto; border-top:1px solid var(--border); padding-top:16px; color:var(--accent-bright);">
                    ...
                    <section class="admin-content">
                                        ...
                                        <tbody>
                                            ...
                                            try {
                                                if ($current_role === 'admin') {
                                                    ...
                                                    $stmt = $pdo->prepare("SELECT * FROM receipts WHERE agency_owner = ? ORDER BY created_at DESC");
                                                    $stmt->execute([$current_user]);
                                                    $all_receipts = $stmt->fetchAll();
                                                }
                                                if (empty($all_receipts)) {
                                                    echo "<tr><td colspan='7' style='text-align:center;'>Chưa có hóa đơn nào</td></tr>";
                                                } else {
                                                    $stt = count($all_receipts);
                                                    foreach ($all_receipts as $r) {
                                                        $r_id = (int) $r['id'];
                                                        $r_user = htmlspecialchars($r['username'], ENT_QUOTES, 'UTF-8');
                                                        $r_img = htmlspecialchars($r['receipt_img'], ENT_QUOTES, 'UTF-8');
                                                        ...
                                                        echo "<td style='vertical-align:middle;'>#{$stt}</td>";
                                                        $stt--;
                                                        echo "<td style='vertical-align:middle;'><strong>{$r_user}</strong><br><span style='font-size:12px;color:var(--text-2);'>Current: " . htmlspecialchars(getRoleLabel($u_role), ENT_QUOTES, 'UTF-8') . "</span>" . ($requested_plan !== '' ? "<br><span style='font-size:12px;color:var(--accent-bright);'>Yêu cầu: {$requested_plan}" . ($requested_amount > 0 ? " (" . number_format($requested_amount) . "đ)" : "") . "</span>" : "") . "</td>";
                                                        echo "<td style='vertical-align:middle;'><a href='javascript:void(0)' onclick=\"Swal.fire({imageUrl: '{$r_img}', imageAlt: 'Hóa đơn', showConfirmButton: false, showCloseButton: true, background: 'transparent', backdrop: 'rgba(0,0,0,0.8)'})\"><img src='{$r_img}' style='width:50px; height:60px; object-fit:contain; border-radius:4px; border:1px solid var(--border); cursor:pointer; background:var(--bg-1);'></a></td>";
                                                        echo "<td style='vertical-align:middle;'><span class='badge {$badge_class}'>{$r_status}</span></td>";
                                                        echo "<td style='vertical-align:middle;'>{$r_date}</td>";
                                                        echo "<td style='vertical-align:middle;'>
                                                            <form method='POST' style='display:flex; align-items:center; justify-content:center; gap:6px; margin:0;'>
                                                                ...
                                                                <select name='role' class='sel' style='padding:4px 8px; font-size:12px; height:32px;'>
                                ...
                                <form method="POST">
                                    ...
                                    <?php if ($current_role === 'agency'): ?>
                                        <div class="field" style="margin-bottom:15px;">
                                            ...
                                            <input type="text" name="site_name" class="input"
                                                value="<?= htmlspecialchars($gs['site_name'] ?? '') ?>"
                                                placeholder="Tên shop của bạn..." required>
                                        </div>
                                        <div class="field" style="margin-bottom:15px;">
                                    ...
                                    <div class="field" style="margin-bottom:15px;">
                                        ...
                                        <input type="text" name="bank_account" class="input"
                                            value="<?= htmlspecialchars($gs['bank_account'] ?? '') ?>" placeholder="0123456789"
                                            required>
                                    </div>
                                    <div class="field" style="margin-bottom:20px;">
                                        <label>Tên Chủ Tài Khoản (Không dấu)</label>
                                        <input type="text" name="bank_owner" class="input"
                                            value="<?= htmlspecialchars($gs['bank_owner'] ?? '') ?>" placeholder="NGUYEN VAN A"
                                            required>
                                    </div>
                                    ...
                                    <div class="field"
                                        ...
                                        <p style="font-size:13px; color:var(--text-2); margin-bottom:10px;">Nhập token lấy từ ThueAPI.Pro để hệ thống tự động kiểm tra lịch sử giao dịch. Để trống nếu không dùng.</p>
                        ...
                        <?php if ($tab == 'dashboard'): ?>
                            <div class="admin-panel">
                                ...
                                <h3 style="margin-bottom: 16px; font-size: 14px; color: var(--text-2); text-transform: uppercase; letter-spacing: 1px;">Biểu đồ Doanh thu (Tuần này - Từ Thứ 2)</h3>
                                <div style="background:var(--bg-0); padding:20px; border-radius:12px; border:1px solid var(--border); margin-bottom: 24px; position: relative; height: 300px;">
                                    <canvas id="revenueWeekChart"></canvas>
                                </div>

                                <h3 style="margin-bottom: 16px; font-size: 14px; color: var(--text-2); text-transform: uppercase; letter-spacing: 1px;">Biểu đồ Doanh thu (Tháng này)</h3>
                                <div style="background:var(--bg-0); padding:20px; border-radius:12px; border:1px solid var(--border); margin-bottom: 24px; position: relative; height: 300px;">
                                ...
                                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(320px, 1fr)); gap:16px;">
                                    <div
                                        ...
                                        <form method="POST" enctype="multipart/form-data">
                                            ...
                                            <div class="field" style="margin-bottom:10px;">
                                                ...
                                                <input type="file" name="guide_video" accept="video/*" class="input"
                                    ...
                                    <div
                                        ...
                                        <?php if ($guide_video_recovery && file_exists($guide_video_recovery)): ?>
                                            ...
                                            <form method="POST" style="margin-bottom:10px;"
                                                onsubmit="return confirm('Xóa video mục 2?');">
                                                <input type="hidden" name="action" value="admin_delete_guide_video_recovery">
                                                <button type="submit" class="action-btn del"
                                                    style="padding:6px 12px; font-size:12px;">Xóa video mục 2</button>
                                            </form>
                                        <?php endif; ?>
                                        <form method="POST" enctype="multipart/form-data">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div style="display:flex; gap:8px;">
                                                            <a href="/bai-viet?slug=<?= htmlspecialchars($art['slug']) ?>"
                                ...
                                <?php if ($total_pages > 1): ?>
                                    <div style="display:flex; justify-content:center; gap:8px; margin-top:20px;">
                                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                            <a href="?tab=articles&p=<?= $i ?>"
                                                class="btn <?= $i == $page_no ? 'btn-primary' : 'btn-outline' ?>"
                            ...
                            <div id="modal-add-article" class="admin-modal">
                                <div class="modal-box" style="max-width:800px; max-height:90vh; overflow-y:auto;">
                                    ...
                                    <form method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="field">
                                            <label>Nội dung chi tiết (Hỗ trợ HTML) <span
                                        ...
                                        <div style="display:flex; gap:12px; margin-top:24px;">
                                            <button type="button" class="btn btn-outline"
                                                onclick="closeModal('modal-add-article')">Hủy bỏ</button>
                            ...
                            <div id="modal-edit-article" class="admin-modal">
                                <div class="modal-box" style="max-width:800px; max-height:90vh; overflow-y:auto;">
                                    ...
                                    <form method="POST" enctype="multipart/form-data">
                                        ...
                                        <div style="display:flex; gap:12px; margin-top:24px;">
                                            <button type="button" class="btn btn-outline"
                                                onclick="closeModal('modal-edit-article')">Hủy bỏ</button>
                            ...
                            <div class="admin-panel">
                                ...
                                <form method="POST" enctype="multipart/form-data"
                                    ...
                                    <div class="field" style="margin-bottom: 12px;">
                                        ...
                                        <input type="file" name="feedback_image" accept="image/*" class="input"
                                            style="padding: 12px;" required>
                                    </div>
                        ...
                        <?php if ($tab == 'branding'): ?>
                            <div class="admin-panel">
                                ...
                                <form method="POST" enctype="multipart/form-data"
                                    ...
                                    <!-- ── Tên & SEO ── -->
                                    <div
                                        style="background:var(--bg-2);border:1px solid var(--border);border-radius:16px;padding:24px;">
                                        <h3
                                        ...
                                        <div style="display:flex;flex-direction:column;gap:14px;">
                                            <div class="field">
                                                ...
                                                <input type="text" name="site_name" class="input"
                                                    value="<?= htmlspecialchars($cur_name) ?>" placeholder="VD: Locket Gold VN">
                                            </div>
                                            <div class="field">
                                        ...
                                        <div style="display:grid;grid-template-columns:auto 1fr;gap:20px;align-items:center;">
                                            ...
                                            <div>
                                                <div style="font-size:12px;color:var(--text-2);margin-bottom:8px;">Ảnh hiện tại:
                                                    <code
                                                        style="color:var(--accent-bright);"><?= htmlspecialchars($cur_logo) ?></code>
                                                </div>
                                                <div class="field" style="margin:0;">
                                                    <label style="font-size:13px;">Thay logo mới <small
                                                            style="color:var(--text-2);">(PNG, JPG, SVG, WebP — nên dùng
                                    ...
                                    <div
                                        ...
                                        <div style="display:grid;grid-template-columns:auto 1fr;gap:20px;align-items:center;">
                                            ...
                                            <div>
                                                <div style="font-size:12px;color:var(--text-2);margin-bottom:8px;">Ảnh hiện tại:
                                                    <code
                                                        style="color:var(--accent-bright);"><?= htmlspecialchars($cur_fav) ?></code>
                                                </div>
                                                <div class="field" style="margin:0;">
                                    ...
                                    <div
                                        ...
                                        <div class="field" style="margin:0;">
                                            ...
                                            <input type="file" name="banner_file" accept="image/*" class="input"
                                                style="padding:10px;">
                                        </div>
                                    </div>
                        ...
                        <?php if ($tab == 'users'): ?>
                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach ($pdo->query("SELECT * FROM users ORDER BY id DESC")->fetchAll() as $u): ?>
                                                <tr>
                                                    ...
                                                    <td data-label="Thời gian ĐK" style="color:var(--text-2); font-size:13px; white-space: nowrap;">
                                                    ...
                                                    <td class="td-actions" data-label="Thao tác" style="white-space:nowrap;">
                                                        <?php if ($u['username'] !== $current_user): ?>
                                                            ...
                                                            <button class="action-btn" title="Chỉnh sửa thông tin"
                                                                onclick="editUser(<?= $u['id'] ?>, '<?= htmlspecialchars($u['username']) ?>', '<?= $u['role'] ?>')"><svg
                                                            ...
                                                            <form method="POST" style="display:inline;"
                                                                ...
                                                                <input type="hidden" name="action" value="admin_del_user"><input
                            ...
                            <div id="modal-add-user" class="admin-modal">
                                <div class="modal-box">
                                    ...
                                    <form method="POST">
                                        ...
                                        <div class="field"><label>Tên đăng nhập</label><input type="text" name="username"
                                                class="input" required></div>
                                        <div class="field"><label>Mật khẩu</label><input type="text" name="password" class="input"
                                                required></div>
                            ...
                            <div id="modal-edit-user" class="admin-modal">
                                <div class="modal-box">
                                    <h3 class="heading-sm mb-md">Sửa tài khoản <span id="e_uname"
                                            style="color:var(--accent-bright)"></span></h3>
                                    <form method="POST">
                            ...
                            <script>
                                function editUser(id, un, role) { document.getElementById('e_id').value = id; document.getElementById('e_uname').innerText = un; document.getElementById('e_role').value = role; showModal('modal-edit-user'); }
